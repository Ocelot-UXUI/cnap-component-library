...
提供批量删除重建 Pod 的确认弹窗：用户在 Pod 列表页选中 Pod 后，弹窗展示待操作 Pod 清单预览和集群参数（只读），用户确认后执行删除重建操作（先删除 Pod，再重建 Pod）。

## In Scope

- 弹窗布局：标题栏、操作提示区、待删除重建 Pod 列表（只读预览）、集群参数配置（只读）、底部操作栏
- 弹窗尺寸：宽度 800px，高度自适应
- 操作提示区：使用 antd Alert 组件（warning 类型）展示删除重建关键影响
- Pod 列表：只读表格，列含 Pod 名称、所属工作负载、集群、状态
- 集群参数区：由所选 Pod 聚合集群，从对应 Workload 的 `updateStrategy` 和 `availabilityTarget` 读取参数，只读展示
- 提交接口 `runtimeOperationApi.deletePod()` 请求体组装
- 提交成功后关闭弹窗，展示成功提示含"查看执行详情"链接
- 从 Pod 列表页 BatchActionBar 的"批量删除重建"按钮触发

## Out Of Scope

- 单个 Pod 的删除重建（Pod 行内操作）
- 操作进度追踪/状态轮询
- 操作的撤销/回滚
- 操作历史/审计日志
- 取消正在进行的操作
...
6. 成功后：弹窗关闭 → 展示成功提示"批量删除重建命令已下发，查看执行详情"（含链接）
...
- **删除重建行为**：操作依次执行：先删除 Pod，再重建 Pod。Pod 名称、IP、所在节点等属性会发生变化
- **Evicted 状态 Pod**：处于已驱逐状态的 Pod 会被彻底删除（不再创建新 Pod）；其他状态的 Pod 会创建新的 Pod
- **提交成功提示**：接口成功返回后提示"批量删除重建命令已下发，查看执行详情"，其中"查看执行详情"为链接，链接地址暂未确定
...
- [ ] Pod 列表页 BatchActionBar 的"批量删除重建"按钮可点击并打开弹窗
- [ ] 弹窗标题"批量删除重建Pod"，展示环境上下文信息
- [ ] 标题下方展示功能说明文案（描述删除重建行为）
- [ ] 操作提示区使用 Alert 组件（warning 类型）展示关键影响
- [ ] 待删除重建 Pod 列表正确展示所选 Pod（名称、工作负载、集群、状态），数量与标题计数一致
- [ ] Pod 列表和集群参数均为只读，不可编辑
- [ ] 集群参数正确从 Pod → WorkloadGroup → RuntimeWorkload 数据链获取，按集群分组展示
- [ ] 可用度为空时显示"未启用"
- [ ] Evicted 状态 Pod 以 Tag 正确标识
- [ ] 提交成功后弹窗关闭，展示提示"批量删除重建命令已下发，查看执行详情"，含链接
...
弹窗尺寸：800px 宽，高度自适应。Modal 弹窗，居中展示。

#### 标题栏

- 标题："批量删除重建Pod"
- 环境信息：格式"环境 : {环境名称}"
- 功能说明文案："删除重建操作将触发集群释放当前 Pod 并重新申请、重新启动的过程"

#### 操作提示区

使用 antd Alert 组件（warning 类型），展示两条提示：

1. 删除重建过程中会销毁当前 Pod 并创建新的 Pod；Pod 名称、IP、所在节点等属性会发生变化
2. 处于已驱逐（Evicted）状态的 Pod 会被彻底删除；其他状态的 Pod 会创建一个新的 Pod

#### 待删除重建 Pod 列表

标题"待删除重建Pod"后跟 Pod 数量。
...
| 所属工作负载 | workloadName                           |
| 集群         | 集群图标 + clusterName                 |
| 状态         | Tag 组件（运行中=绿色, 已驱逐=红色等） |

#### 集群与参数配置

标题"集群与参数配置"，说明"集群与参数配置由系统自动推导，所有参数均不可修改"。

集群参数来源链路：`Pod.workloadName → WorkloadGroup.id (groupId) → GET /runtime/workloads?groupId → RuntimeWorkload[].updateStrategy / availabilityTarget`

按 Pod 的 `clusterId` 去重聚合集群后，每个集群匹配对应的 RuntimeWorkload 数据。

| 列         | 说明                         | 数据来源                                          |
| ---------- | ---------------------------- | ------------------------------------------------- |
| 集群       | 集群图标 + 集群名称          | Pod → `clusterId` 去重                            |
| 最大不可用 | 只读百分比, `maxUnavailable` | RuntimeWorkload → `updateStrategy.maxUnavailable` |
| 最大可超出 | 只读百分比, `maxSurge`       | RuntimeWorkload → `updateStrategy.maxSurge`       |
| 可用度     | 只读百分比, 空→"未启用"      | RuntimeWorkload → `availabilityTarget`            |
...
提交时调用封装的 API 方法，参照源码 `src/api/runtimeOperation.ts:183`：
...
runtimeOperationApi.deletePod({
    ...
    force: false, // 可选，true 为强制删除（pod.delete-force）
});
```

普通删除 `operation: 'pod.delete'`，`force: true` 时使用 `operation: 'pod.delete-force'`。`targets[].resourceType` 默认为 `'v1/pods'`。
...
| `POST /rest/v1/application-environments/:appEnvID/runtime/operations/pod.delete/trigger` | 触发删除重建                                               |
